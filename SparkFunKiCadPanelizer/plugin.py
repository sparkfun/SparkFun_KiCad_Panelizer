...
class PanelizerPlugin(pcbnew.ActionPlugin, object):
    ...
    def Run(self):
        ...
        panelOutputPath = os.path.split(board.GetFileName())[
            0
        ]  # Get the file path head
        panelOutputPath = os.path.join(
            panelOutputPath, self.productionDir
        )  # Add the production dir
        ...
        self.ordering_instructions = os.path.join(
            panelOutputPath, "ordering_instructions.txt"
        )
        ...
        for i in range(numlayers):
            layertable[i] = {
                "standardName": board.GetStandardLayerName(i),
                "actualName": board.GetLayerName(i),
            }
        ...
        def run_panelizer(dlg, p_panelizer):
            ...
            if not self.IsSupported():
                ...
                self.logger.log(
                    logging.WARNING,
                    'Version check failed. "{}" may not be supported. Panelizing may fail'.format(
                        self.kicad_build_version
                    ),
                )
            ...
            command.extend(
                ["--vscorelayer", vscorelayer, "--vscoretextlayer", vscorelayer]
            )

            gapx = float(dlg.CurrentSettings()["gapsVerticalCtrl"]) * convertDimensions
            gapy = (
                float(dlg.CurrentSettings()["gapsHorizontalCtrl"]) * convertDimensions
            )
            ...
            if board is not None:
                sysExit, report = p_panelizer.startPanelizerCommand(
                    command, board, self.ordering_instructions, self.logger
                )
                ...
                if sysExit > 0:
                    wx.MessageBox(
                        "Panelizer "
                        + ("warning" if (sysExit == 1) else "error")
                        + ".\nPlease check panelizer.log for details.",
                        ("Warning" if (sysExit == 1) else "Error"),
                        wx.OK | (wx.ICON_WARNING if (sysExit == 1) else wx.ICON_ERROR),
                    )
                else:
                    wx.MessageBox(
                        "Panelizer complete.\nPlease check panelizer.log for details.",
